...
class Pix2PixModel(torch.nn.Module):
  ...
  def forward(self, data, mode):
    ...
    if not self.opt.FiveK:
      ...
      if mode == 'generator':
        ...
        with torch.no_grad():
          context_embs, sent_emb, words_embs = self.text_encoder(captions, cap_lens, max_len)
          fake_image, ret_dict = self.generate_fake_my(input_semantics=input_semantics, input_image=input_image,
                                                        sent_emb=sent_emb, words_embs=words_embs, context_embs=context_embs)
          # for key in ret_dict:
          #   ret_dict[key] = ret_dict[key].detach()
          return fake_image.detach(), ret_dict
      ...
      if mode == 'generator':
        g_loss, generated = self.compute_generator_loss_my(input_image=input_image, output_image=output_image,
                                                           context_embs=context_embs, sent_emb=sent_emb,
                                                           words_embs=words_embs)
        return g_loss, generated
      elif mode == 'discriminator':
        d_loss = self.compute_discriminator_loss_my(input_image=input_image, output_image=output_image,
                                                    context_embs=context_embs, sent_emb=sent_emb,
                                                    words_embs=words_embs)
        return d_loss
      elif mode == 'inference':
        with torch.no_grad():
          context_embs, sent_emb, words_embs = self.text_encoder(captions, cap_lens, max_len)
          fake_image, ret_dict = self.generate_fake_my(input_image=input_image, sent_emb=sent_emb,
                                                       words_embs=words_embs, context_embs=context_embs)
          # for key in ret_dict:
          #   ret_dict[key] = ret_dict[key].detach()
          return fake_image.detach(), ret_dict
  ...
  # def compute_generator_loss_my(self, input_semantics, input_image, output_image, glove_words_embed=None, labels_embed=None):
  ...
  def compute_generator_loss_my(self, **input_G):
    ...
    if self.opt.use_pattn:
      if self.opt.FiveK:
        fake_image, ret_dict = self.netG(input_semantics=None,
                                  input_image=input_G['input_image'],
                                  sent_emb=input_G['sent_emb'], words_embs=input_G['words_embs'],
                                  context_embs=input_G['context_embs'])
      else:
        fake_image, ret_dict = self.netG(input_semantics=input_G['input_semantics'],
                                  input_image=input_G['input_image'],
                                  sent_emb=input_G['sent_emb'], words_embs=input_G['words_embs'],
                                  context_embs=input_G['context_embs'])
    else:
      fake_image = self.netG(input_image=input_G['input_image'], sent_emb=input_G['sent_emb'])
      condition = input_G['sent_emb']

    if self.opt.lambda_gan > 0:
      pred_fake, pred_real = self.discriminate(condition, fake_image=fake_image, real_image=input_G['output_image'])
      G_losses['GAN_cond'] = self.criterionGAN(pred_fake, True, for_discriminator=False) * self.opt.lambda_gan
    ...
    if self.opt.lambda_MSE:
      G_losses['MSE'] = self.opt.lambda_MSE * self.criterionMSE(fake_image, input_G['output_image'])
  ...
  def compute_discriminator_loss_my(self, **input_D):
    ...
    with torch.no_grad():   # ！！！大错误，把这里弄错了，G的输入应该是input_image而非output_image
      # input_semantics = input_semantics.detach()
      # input_image = input_image.detach()
      # if self.opt.use_op:
      #   fake_image = self.netG(input_semantics=input_D['input_semantics'], input_image=input_D['input_image'])
      #   condition = input_D['input_semantics']

      if self.opt.FiveK:
        fake_image, ret_dict = self.netG(input_semantics=None,
                                  input_image=input_D['input_image'],
                                  sent_emb=input_D['sent_emb'], words_embs=input_D['words_embs'],
                                  context_embs=input_D['context_embs'])
      else:
        fake_image, ret_dict = self.netG(input_semantics=input_D['input_semantics'],
                                  input_image=input_D['input_image'],
                                  sent_emb=input_D['sent_emb'], words_embs=input_D['words_embs'],
                                  context_embs=input_D['context_embs'])
      # else:
      #   # self.opt.lang_encoder == 'bilstm'
      #   fake_image = self.netG(input_image=input_D['input_image'], sent_emb=input_D['sent_emb'])
      #   condition = input_D['sent_emb']
    ...
    # fake_image.requires_grad_()
    # if self.opt.lambda_gan > 0:
    #   pred_fake, pred_real = self.discriminate(condition, fake_image, input_D['output_image'])
    #   D_losses['D_Fake_cond'] = self.criterionGAN(pred_fake, False, for_discriminator=True) * self.opt.lambda_gan
    #   D_losses['D_real_cond'] = self.criterionGAN(pred_real, True, for_discriminator=True) * self.opt.lambda_gan

    if self.opt.lambda_gan_uncond > 0:
      # pred_fake, pred_real = self.discriminate(None, fake_image, input_D['output_image'])
      pred_fake, pred_real = self.discriminate(None, fake_image * ret_dict['masks'], input_D['output_image'] * ret_dict['masks'])
