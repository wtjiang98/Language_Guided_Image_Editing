...
# def getInput(input_path, request):
#   input_img = Image.open(input_path).convert('RGB')
#   params = get_params(opt, input_img.size)
...
for i, data_i in enumerate(tqdm(dataloader)):
  ...
  # (b x h x w x c)
  generated_array = util.tensor2im(generated)
  util.save_image(generated_array[0], opt.output_path, create_dir=False)
